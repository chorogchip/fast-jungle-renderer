#include "JungleSceneBuilder.hpp"
...
namespace fjr::cooker {
    ...
    namespace {
        ...
        using internal::StaticSceneDataBuilder;
        using internal::UsdCameraBuilder;
        using internal::UsdEnvironmentBuilder;
        using internal::UsdMaterialBuilder;
        using internal::UsdMeshBuilder;
        using log::fail;
		...
		[[nodiscard]] bool matrix_bits_equal(
			...
			return std::bit_cast<std::array<uint32_t, 16>>(left) ==
				std::bit_cast<std::array<uint32_t, 16>>(right);
		}
        ...
        public:
            explicit Builder(pxr::UsdStageRefPtr stage)
                : stage_(std::move(stage)),
                  ...
                  material_builder_(scene_builder_),
                  mesh_builder_(converter_, scene_builder_, material_builder_),
                  result_(&scene_builder_.storage()) {}
        ...
        private:
				...
				std::ranges::stable_sort(
					...
					[](const PointInstancerSource& left,
					   const PointInstancerSource& right) {
						return std::tuple{
							left.mesh,
							static_cast<uint32_t>(left.category),
							left.source_ordinal,
						} < std::tuple{
							right.mesh,
							static_cast<uint32_t>(right.category),
            ...
            void build_direct_scene() {
                for (const auto& prim : stage_->Traverse()) {
                    ...
                    if (prim.IsA<pxr::UsdGeomMesh>() && is_visible(prim)) {
						...
						const auto mesh =
							mesh_builder_.build(prim).value();
			...
			void flip_mesh_facing(uint32_t mesh_index) {
			...
			void collect_single_mesh(
                ...
                if (prim.IsA<pxr::UsdGeomMesh>()) {
					...
					result.mesh = mesh_builder_.build(prim).value();
    ...
    std::unique_ptr<scene::StaticScene> internal::JungleSceneBuilder::build(
