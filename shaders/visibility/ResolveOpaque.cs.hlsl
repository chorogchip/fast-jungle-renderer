...
void main(uint3 tid : SV_DispatchThreadID)
{
    ...
    if (software_winner)
    {
        ...
        const uint local_work =
            ...
            (SOFTWARE_LOCAL_WORK_CAPACITY - 1u);
    ...
    else
    {
        ...
        submesh = submeshes[submesh_id];
        const uint index_offset = submesh.index_offset +
    ...
    const uint raster_class = submesh.raster_class;
    if (raster_class > RASTER_CLASS_ALPHA)
        return;
    
    const uint material_id = submesh.material_id;
