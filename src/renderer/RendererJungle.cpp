...
namespace fjr::render {
    ...
    void RendererJungle::init(
        ...
        for (uint32_t frame = 0; frame < FRAME_COUNT; ++frame) {
            software_resources.frames[frame] =
                ...
                .batch_count = data_per_frame_[frame]
                    .software_batch_count.get(),
            };
        }
        software_resources.instances = data_persistant_.instance_transform
            ->GetGPUVirtualAddress();
        software_resources.vertex_decode_params = data_persistant_
            .vertex_decode_params->GetGPUVirtualAddress();
        software_resources.submeshes = data_persistant_.submesh
            ->GetGPUVirtualAddress();
        software_resources.raster_clusters = data_persistant_
            .raster_cluster->GetGPUVirtualAddress();
        software_resources.raster_cluster_vertices = data_persistant_
            .raster_cluster_vertices->GetGPUVirtualAddress();
        software_resources.raster_cluster_triangles = data_persistant_
    ...
    void RendererJungle::update_software_resolve_views() {
        for (uint32_t frame = 0; frame < FRAME_COUNT; ++frame) {
            device_->CopyDescriptorsSimple(
                ...
                sw_raster_pass_.get_key_srv(frame).get_cpu(),
                D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
            device_->CopyDescriptorsSimple(
                1,
    ...
    void RendererJungle::render() {
        ...
        command_queue_.wait(
            resolve_contexts_[frame].get_fence_value());
        ...
        auto& software_context = compute_contexts_[frame];
