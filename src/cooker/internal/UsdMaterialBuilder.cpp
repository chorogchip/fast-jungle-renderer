...
namespace fjr::cooker::internal {
    ...
    UsdMaterialBuilder::UsdMaterialBuilder(
        StaticSceneDataBuilder& scene_builder)
        : scene_builder_(scene_builder) {}

    MaterialProduct UsdMaterialBuilder::build(
        ...
        material.name = scene_builder_.intern_string(
        ...
        product.id = scene_builder_.append_material(material);
        cache_.emplace(key, product);
        return product;
    }

    pxr::UsdShadeShader UsdMaterialBuilder::find_surface_shader(
    ...
    pxr::UsdShadeInput UsdMaterialBuilder::find_input(
        const pxr::UsdShadeShader& shader,
    ...
    float UsdMaterialBuilder::read_float(
        const pxr::UsdShadeShader& shader,
        std::string_view name,
        float fallback) {
    ...
    DirectX::XMFLOAT3 UsdMaterialBuilder::read_float3(
        const pxr::UsdShadeShader& shader,
        std::string_view name,
    ...
    DirectX::XMFLOAT4 UsdMaterialBuilder::read_float3_as_color(
        const pxr::UsdShadeShader& shader,
        std::string_view name,
        const DirectX::XMFLOAT3& fallback) {
    ...
    bool UsdMaterialBuilder::resolve_material_texture(
    ...
    TextureSampleSemantic UsdMaterialBuilder::resolve_texture_sample(
    ...
    TextureBindingId UsdMaterialBuilder::commit_texture_sample(
        const TextureSampleSemantic& sample) {

        StaticScene::TextureBinding binding;
        binding.texture = scene_builder_.intern_texture(sample.asset).value();
        binding.sampler = scene_builder_.intern_sampler(sample.sampler).value();
        binding.channel = sample.components;
        binding.flags = sample.color_space;
        return scene_builder_.append_texture_binding(binding);
    }

    std::string UsdMaterialBuilder::read_token_or_string(
    ...
    UsdMaterialBuilder::texture_address_mode(std::string_view value) {
    ...
    UsdMaterialBuilder::texture_components(
