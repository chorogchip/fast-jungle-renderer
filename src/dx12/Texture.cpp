...
namespace fjr::dx {
    ...
    void Texture::init(
        ...
        this->set_resource(std::move(resource), initial_state);
        type_ = type;
    }
    ...
    void Texture::attach(
        ...
        this->set_resource(Microsoft::WRL::ComPtr<ID3D12Resource>(resource), initial_state);
        type_ = type;
    }
    ...
    void Texture::create_srv(
        ...
        case TextureType::texture2d_array:
            if (resource.SampleDesc.Count > 1) {
                description.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DMSARRAY;

                description.Texture2DMSArray.FirstArraySlice = range.first_slice;
                description.Texture2DMSArray.ArraySize = range.slice_count;
            } else {
                description.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2DARRAY;

                description.Texture2DArray.MostDetailedMip = range.first_mip;
                description.Texture2DArray.MipLevels = range.mip_count;
                description.Texture2DArray.FirstArraySlice = range.first_slice;
    ...
    void Texture::create_rtv(
        ...
        case TextureType::texture2d:
            if (resource.SampleDesc.Count > 1) {
                description.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2DMS;
            } else {
        ...
        case TextureType::texture_cube_array:
            if (resource.SampleDesc.Count > 1) {
                description.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2DMSARRAY;

                description.Texture2DMSArray.FirstArraySlice = first_slice;
                description.Texture2DMSArray.ArraySize = slice_count;
            } else {
                description.ViewDimension = D3D12_RTV_DIMENSION_TEXTURE2DARRAY;
    ...
    void Texture::create_dsv(
        ...
        case TextureType::texture2d:
            if (resource.SampleDesc.Count > 1) {
                description.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2DMS;
            } else {
        ...
        case TextureType::texture_cube_array:
            if (resource.SampleDesc.Count > 1) {
                description.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2DMSARRAY;

                description.Texture2DMSArray.FirstArraySlice = first_slice;
                description.Texture2DMSArray.ArraySize = slice_count;
            } else {
                description.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2DARRAY;
