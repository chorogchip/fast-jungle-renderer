...
namespace fjr::cooker::internal {
    ...
    class UsdMeshCompiler::Impl final {
    public:
        Impl(
            ...
            : converter_(converter),
              ...
              material_compiler_(material_compiler),
              result_(&assembler.storage()) {}
        ...
        [[nodiscard]] std::uint32_t add_string(std::string_view value) {
            ...
            [[nodiscard]] std::uint32_t cook_mesh(
                ...
                for (const auto& group : groups) {
                    const auto material = material_compiler_.compile(
                        group.material_path,
                        group.material);
    ...
    UsdMeshCompiler::UsdMeshCompiler(
        SceneSpace& converter,
        StaticSceneAssembler& assembler,
        UsdMaterialCompiler& material_compiler)
        : impl_(std::make_unique<Impl>(
            converter, assembler, material_compiler)) {}

    UsdMeshCompiler::~UsdMeshCompiler() = default;

    MeshId UsdMeshCompiler::compile(const pxr::UsdPrim& prim) {
        return impl_->compile(prim);
    }
