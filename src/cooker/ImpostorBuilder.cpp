...
namespace fjr::cooker {
    namespace {
        ...
        constexpr uint32_t DIRECTION_COUNT = 8;
        constexpr float LOD_ENTRY_PIXELS = 4.0f;
        constexpr float TEXELS_PER_ENTRY_PIXEL = 1.25f;
        constexpr uint32_t BORDER_TEXELS = 4;
        constexpr uint32_t DIMENSION_ALIGNMENT = 16;
        ...
        struct BakeTarget final {
            uint32_t mesh = StaticScene::INVALID_INDEX;
            std::string name;
            Bounds bounds;
            float radius = 0.0f;
        ...
        struct BakeMaterialConstants final {
            ...
            uint32_t base_color_texture = StaticScene::INVALID_INDEX;
            uint32_t opacity_texture = StaticScene::INVALID_INDEX;
            uint32_t base_color_channel = 0;
            uint32_t opacity_channel = 0;
            float roughness_value = 0.5f;
            uint32_t roughness_texture = StaticScene::INVALID_INDEX;
            uint32_t roughness_channel = 0;
            uint32_t normal_texture = StaticScene::INVALID_INDEX;
            uint32_t render_back_faces = 0;
        };
        ...
        [[nodiscard]] uint32_t append_string(
            StaticScene& scene,
        ...
        [[nodiscard]] uint32_t ensure_sampler(StaticScene& scene) {
        ...
        [[nodiscard]] uint32_t aligned_dimension(float desired) {
            const auto unaligned = static_cast<uint32_t>(std::ceil(
        ...
        [[nodiscard]] uint32_t texture_from_binding(
            const StaticScene& scene,
            uint32_t binding) {
        ...
        [[nodiscard]] BakeMaterialConstants material_constants(
            ...
            const auto binding = [&scene](uint32_t binding_index)
            ...
            result.render_back_faces =
                (static_cast<uint32_t>(submesh_flags) &
        ...
        void upload_source_texture(
            ...
            UploadedTexture& destination) {
            auto source_texture = TextureSourceBuilder::build(path);
            const auto& metadata = source_texture.metadata;
            ...
            for (UINT mip = 0; mip < metadata.mipLevels; ++mip) {
                const auto* source = source_texture.image.GetImage(mip, 0, 0);
                if (source == nullptr) {
                    log::Logger::g_logger << log::abrt(
        ...
        [[nodiscard]] std::vector<BakedTarget> bake_targets(
            ...
            std::vector<std::optional<UploadedTexture>> source_textures(
                scene.textures.size());
            for (uint32_t texture = 0;
                ...
                upload_source_texture(
                    ...
                    uploader,
                    std::filesystem::path{
                        texture_source_key(scene, texture)},
            ...
            root_builder.set_constants(RootParameter::CAMERA)
                .reg(0)
                .count(sizeof(BakeCameraConstants) / sizeof(uint32_t))
                .vis_vertex()
                .add();
            root_builder.set_constants(RootParameter::MATERIAL)
                .reg(1)
                .count(sizeof(BakeMaterialConstants) / sizeof(uint32_t))
                .vis_pixel()
                .add();
            ...
            pipeline_desc.RTVFormats[2] = DXGI_FORMAT_R8_UNORM;
            ...
            rtv_heap.init(device.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_RTV, 3, false);
            ...
            for (std::size_t target_index = 0;
                ...
                for (uint32_t direction_index = 0;
                    ...
                    color.init(device.Get(), color_desc,
                        D3D12_RESOURCE_STATE_RENDER_TARGET, &color_clear);
                    normal.init(device.Get(), normal_desc,
                        D3D12_RESOURCE_STATE_RENDER_TARGET, &normal_clear);
                    roughness.init(device.Get(), roughness_desc,
                    ...
                    // fresh three-RTV/one-DSV heap is the simple safe reuse.
                    rtv_heap.init(device.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_RTV, 3, false);
                    dsv_heap.init(device.Get(), D3D12_DESCRIPTOR_HEAP_TYPE_DSV, 1, false);
                    const auto rtvs = rtv_heap.alloc(3);
                    ...
                    normal.create_rtv(device.Get(), rtvs.get_cpu(1), 0, 0, 1,
                        DXGI_FORMAT_R8G8B8A8_UNORM);
                    roughness.create_rtv(device.Get(), rtvs.get_cpu(2), 0, 0, 1,
                        DXGI_FORMAT_R8_UNORM);
                    ...
                    const std::array<D3D12_CPU_DESCRIPTOR_HANDLE, 3> rtv_handles{
                        rtvs.get_cpu(0), rtvs.get_cpu(1), rtvs.get_cpu(2) };
                    ...
                    context->ClearRenderTargetView(rtvs.get_cpu(2), clear_roughness.data(), 0, nullptr);
                    ...
                    context->SetGraphicsRoot32BitConstants(
                        ...
                        sizeof(camera) / sizeof(uint32_t), &camera, 0);
                    ...
                    context.transition(color, D3D12_RESOURCE_STATE_COPY_SOURCE);
                    context.transition(normal, D3D12_RESOURCE_STATE_COPY_SOURCE);
                    context.transition(roughness, D3D12_RESOURCE_STATE_COPY_SOURCE);
        ...
        void append_scene_assets(
            ...
            for (std::size_t target_index = 0;
                ...
                uint32_t roughness_start = StaticScene::INVALID_INDEX;
                if (target.bake_roughness) {
                ...
                for (uint32_t direction = 0;
                    ...
                    uint32_t roughness_binding = StaticScene::INVALID_INDEX;
    ...
    std::vector<GeneratedTexture> ImpostorBuilder::build(
        scene::StaticScene& scene) {
        const auto targets = plan_targets(scene);
        auto baked = bake_targets(scene, targets);
        std::vector<GeneratedTexture> generated_textures;
        append_scene_assets(scene, targets, baked, generated_textures);
